...
@task_route.get("/task", response_model=List[base_objects.AnnotationTask])
async def get_task(
...
@task_route.get("/task_instance_random/:task_id/:result_count", response_model=List[base_objects.AnnotationTaskInstance])
...
@task_route.get("/task_instance/:task_id", response_model=List[base_objects.AnnotationTaskInstance])
async def get_task_instance(task_id: int,
        user_token: TokenData = Depends(get_current_admin), db: AsyncSession = Depends(get_async_session)):
    return await crud_task.get_task_instance(db, task_id)


@task_route.post("/task_instance_result")
...
@task_route.put("/task_instance_result")
